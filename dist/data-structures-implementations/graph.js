...
class Graph {
    ...
    removeEdge(vertex1, vertex2) {
        ...
        const adjacencyList2 = this.adList.get(vertex2);
        adjacencyList1 &&
            this.adList.set(vertex1, adjacencyList1.filter((x) => x !== vertex2));
        adjacencyList2 &&
            this.adList.set(vertex2, adjacencyList2.filter((x) => x !== vertex1));
    }
    removeVertex(name) {
        this.adList.has(name) && this.adList.delete(name);
        for (let key of this.adList.keys()) {
            let adjacencyList = this.adList.get(key);
            this.adList.set(key, adjacencyList.filter((neighbor) => neighbor !== name));
        }
    }
    DFSrecursive(vertex) {
